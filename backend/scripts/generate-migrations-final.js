...
class FinalMigrationGenerator {
  ...
  async generate() {
    console.log('🔍 Analisando Models e Gerando Migrations (Versão Final)...\n');

    await this.loadModels();
    await this.loadMigrations();
    
    // Remove migrations antigas geradas automaticamente
  ...
  async cleanupOldMigrations() {
    const migrationFiles = fs.readdirSync(this.migrationsPath)
      .filter(file => file.startsWith('20251218') && file.includes('-create-') && file.endsWith('.ts'));
  ...
  extractColumnsFromModel(content) {
    ...
    // Usa uma expressão regular mais precisa para encontrar propriedades com decorators
    const propertyRegex = /(?:@[A-Z][a-zA-Z]+\([^)]*\)\s*\n\s*)*?[ \t]*@?[ \t]*(\w+)(\?)?:\s*[^;]+;/gm;
    const matches = content.match(propertyRegex);
    
    if (matches) {
      for (const match of matches) {
        // Ignora getters
        if (match.includes('get ') || match.includes('get(')) continue;
        ...
        // Extrai informações dos decorators
        this.extractColumnDecorators(match, column);
  ...
  extractColumnDecorators(propertyText, column) {
    // Procura por decorators no texto da propriedade
    const decorators = propertyText.match(/@[A-Z][a-zA-Z]+(?:\([^)]*\))?/g) || [];
  ...
  generateMigrationContent(model) {
    // Garante que sempre tenha colunas básicas
    ...
    // Garante que tenha id se não tiver primary key
    if (!model.columns.find(col => col.primaryKey)) {
      model.columns.unshift({ name: 'id', type: 'INTEGER', primaryKey: true, autoIncrement: true, allowNull: false });
    }
    
    // Garante que tenha timestamps se não tiver
    if (!model.columns.find(col => col.name === 'createdAt')) {
      model.columns.push({ name: 'createdAt', type: 'DATE', allowNull: false });
    }
    if (!model.columns.find(col => col.name === 'updatedAt')) {
      model.columns.push({ name: 'updatedAt', type: 'DATE', allowNull: false });
    }
...
if (require.main === module) {
  const generator = new FinalMigrationGenerator();
  generator.generate().catch(console.error);
}

module.exports = FinalMigrationGenerator;
